...
std::vector<ObjectDetectionResult> NNTest::object_result_vector;
float NNTest::xscale;
float NNTest::xoffset;
float NNTest::yscale;
float NNTest::yoffset;
uint8_t NNTest::use_roi;

void (*NNTest::OD_user_CB)(std::vector<ObjectDetectionResult>);
...
void NNTest::configVideo(VideoSetting& config) {
...
void NNTest::configRegionOfInterest(int xmin, int xmax, int ymin, int ymax) {
...
void NNTest::configThreshold(float confidence_threshold, float nms_threshold) {
    od_confidence_thresh = confidence_threshold;
    od_nms_thresh = nms_threshold;
}

void NNTest::begin(void) {
	printf("NNTest begin\r\n");
    if (_p_mmf_context == NULL) {
        _p_mmf_context = mm_module_open(&vipnn_module);
    }
    if (_p_mmf_context == NULL) {
        printf("NNTest init failed\r\n");
        return;
    }
...
void NNTest::end(void) {
    ...
    if (mm_module_close(_p_mmf_context) == NULL) {
        ...
        printf("NNTest deinit failed\r\n");
    }
}

void NNTest::setResultCallback(void (*od_callback)(std::vector<ObjectDetectionResult>)) {
    OD_user_CB = od_callback;
}

uint16_t NNTest::getResultCount(void) {
    return object_result_vector.size();
}

ObjectDetectionResult NNTest::getResult(uint16_t index) {
...
std::vector<ObjectDetectionResult> NNTest::getResult(void) {
    return object_result_vector;
}

void NNTest::ODResultCallback(void *p, void *img_param) {
    (void)img_param;
    if (p == NULL) {
        return;
    }
